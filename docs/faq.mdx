...
Use the normal Minecraft whitelist / ban system. AutoModpack binds each player's Iroh endpoint/public key to the UUID from Minecraft login, so only players currently allowed by the server can establish the download connection. `online-mode=true` is still strongly recommended.

### How do I remove the first connection verification screen?
Publish a DNSSEC-signed TXT record at `_automodpack.<hostname>` containing `endpoint=v1:<server-endpoint-id>`, then make sure clients connect with that hostname. If AutoModpack uses a second raw bootstrap hostname, publish the same TXT record there too. The full explanation is on the [Identity Verification](technicals/certificate) page.
...
Yes! However, only in game, the modpack update functionality on preload will likely not function because we may be only able to communicate via Minecraft connection.
...
If you want to use a different port, set `bindPort` / `portToSend` in the [config](configuration/server-config) instead of sharing the Minecraft port.
