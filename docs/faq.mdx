...
Usually not. On supported targets, the default `HOLEPUNCH` mode uses the Minecraft Login connection. Otherwise, the default `MAGIC_PACKET` mode can share the Minecraft port when `bindPort` is `-1`.
