...
Just delete file from server, or exclude it from sync, read about `syncedFiles` in [config](configuration/server-config). AutoModpack removes the previously managed client file during the next update when its path, SHA-1 hash, and size match the cumulative ownership ledger.
...
Usually not. The default `HOLEPUNCH` mode uses the Minecraft Login protocol connection (Minecraft 1.19.2 and later). On 1.18.2 AutoModpack uses `MAGIC_PACKET` instead, which can share the Minecraft port when `bindPort` is `-1`.

An explicit `bindPort` starts a dedicated listener for `MAGIC_PACKET` or `DIRECT`. Configure `advertisedEndpointHost` and `advertisedEndpointPort` for the public route clients should use. See the complete matrix in [config](configuration/server-config).
