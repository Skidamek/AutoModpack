...
Usually not. The default `HOLEPUNCH` mode uses the Minecraft Login connection on every AutoModpack target. The explicit `MAGIC_PACKET` mode can share the Minecraft port when `bindPort` is `-1`.
