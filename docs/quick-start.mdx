...
On first connection, AutoModpack verifies the modpack host using a normal CA-signed certificate, DNSSEC authentication for a self-signed certificate, or—when neither automatic method is available—a fingerprint prompt. Read more about [certificate verification](technicals/certificate).
...
- Verify the modpack host automatically with CA trust or DNSSEC for a self-signed certificate, or prompt for a fingerprint when manual verification is needed
