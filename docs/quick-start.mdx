...
- **`allowEditsInFiles`**: Files that clients can edit locally. These files are downloaded once and not updated automatically
  ...
  - **⚠️ Warning:** Removing `/config/**` will make ALL config files non-editable. If you only want specific config folders to be always updated, use exclusions instead:
    ...
    ```
- **`overwriteEditableFiles`**: Editable files to overwrite once when their server version changes. These paths must also match `allowEditsInFiles`; otherwise they have no effect
...
- Files listed in both `allowEditsInFiles` and `overwriteEditableFiles` keep local edits until the server changes its version, then are overwritten with that new version
...
Players can edit these FancyMenu files normally. AutoModpack overwrites them only after the server changes the file's content; local changes alone never trigger an update.
