...
On first connection, you will usually be prompted to verify the server's public key / endpoint ID. This ensures you are connecting to the correct server and not a malicious one. Servers can remove that prompt for new clients by publishing a DNSSEC TXT proof for the endpoint ID. Read more about [server identity verification](technicals/certificate).
...
- Prompt you to verify the server's public key / endpoint ID (first time only)
