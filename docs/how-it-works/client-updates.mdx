...
Everything else downloads from the modpack host over its TLS connection. The host compresses documents with zstd, falling back to gzip; files that are already compressed, such as jars, travel as-is. When a platform link fails, the client falls back to your server automatically.
