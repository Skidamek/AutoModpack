...
Everything else downloads from the modpack host over its TLS connection. Each response negotiates its encoding per request - zstd first, gzip as the fallback - and the compression also runs on pack files, where zstd -3 measurably shaves about 14% off the served bytes even for jars. When a platform link fails, the client falls back to your server automatically.
