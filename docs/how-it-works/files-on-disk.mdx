...
| `host-modpack/<group>/` | Pack files the server itself does not need; `main` is the default group |
| `patch-notes.md` | Notes for the next published generation; consumed when it publishes |
...
`recovered/` holds copies saved from the preservation vault. Pack source files belong in `host-modpack/`, not here.
...
| `history/<modpack-id>/` | The journal mirror for one pack, so the History screen works offline |
