...
<details>
    ...
    You can use a browser-based calculator like [SHA1 File
    Checksum](https://emn178.github.io/online-tools/sha1_checksum.html).
</details>
...
| `shareMinecraftConnection` | `true`  | Reuses the player's Minecraft connection to tunnel Iroh packets during in-game updates instead of opening a separate raw TCP bootstrap socket. Preload/offline updates still use raw-TCP routes. |
| `updateIpsOnEveryStart`    | `false` | Updates addressToSend on startup. Useful for dynamic residential IPs.                                                                                                                            |

If you want to remove the first-connection TOFU screen for new clients, prefer a DNS hostname in `addressToSend` and publish a DNSSEC TXT record for it:

```dns
_automodpack.your-hostname.example. 300 IN TXT "endpoint=v1:<endpoint-id>"
```

If players join with one hostname but AutoModpack advertises a different raw bootstrap hostname, you need a matching `_automodpack` TXT record for both names. Read [Identity Verification](../technicals/certificate) for the exact rules.
