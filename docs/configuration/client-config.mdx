...
> #### **Note:** Loader version syncing matches the server's loader version exactly, not only newer ones. If the server advertises an *older* loader version than the one installed locally, the client will be synced back to match the server. Server owners should keep their `loaderVersion` accurate.
