...
Without this type verification users are vulnablre to Man-In-The-Middle attacks, meaning a user might think they are connecting and downloading modpack from server X while a malicious actor is sitting between the user and the server X, qualietly passing their own maliciously crafted modpack instead of original. This however does not protect users from a geunine server providing a maliciously crafted modpack or the server itself getting stolen from original authors.
