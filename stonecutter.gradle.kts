...
// Fully qualified names (e.g. `pl.skidam.automodpack_core.config.ConfigTools` or `java.util.ArrayList` used inline) bypass the import order and rot silently when packages move, so they are banned: use a proper import instead. Spotless has no built-in "FQN to import" step, so formatApply rewrites the roots that are identical on every Minecraft target (own code plus stdlib) and formatCheck fails on anything left. Loader and Minecraft packages (net.*, com.mojang, ...) are intentionally out of scope for the rewrite: an unconditional import can break another target, so those need a human and a stonecutter-guarded import like VersionedScreen's Tooltip import.
val fqnFixPattern = Regex("""(?<![\w$."])(pl|java|javax)(?:\.[a-z][\w]*)*\.([A-Z][\w]*)\b""")
val fqnAnyPattern = Regex("""(?<![\w$."])(?:[a-z][\w]*\.){2,}[A-Z][\w]*\b""")
val javaPackagePattern = Regex("""(?m)^\s*package\s+([\w.]+)\s*;""")
val javaImportPattern = Regex("""(?m)^\s*import\s+(static\s+)?([\w.]+?)(\.\*)?\s*;""")
val javaTypeDeclPattern = Regex("""\b(?:class|interface|enum|record)\s+([A-Z][\w]*)""")

fun maskJavaNoise(content: String): String {
