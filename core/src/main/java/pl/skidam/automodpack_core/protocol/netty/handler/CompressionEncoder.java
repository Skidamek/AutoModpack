...
public class CompressionEncoder extends MessageToByteEncoder<ByteBuf> {
	...
	/**
	 * Pre-sizes the frame the same way the worker does in {@code FileSend}, so a large
	 * payload never grows the buffer by doubling. Exact for single-frame payloads; a multi-chunk message only
	 * grows once past it.
	 */
	@Override
	protected ByteBuf allocateBuffer(ChannelHandlerContext ctx, ByteBuf msg, boolean preferDirect) {
		return ctx.alloc().ioBuffer(ProtocolFrameCodec.HEADER_BYTES + NettyServer.compressionCodec(ctx.channel()).maxCompressedLength(msg.readableBytes()));
	}
	...
	protected void encode(ChannelHandlerContext ctx, ByteBuf msg, ByteBuf out) throws Exception {
		...
		ProtocolFrameCodec.write(out, NettyServer.compressionCodec(ctx.channel()), msg, chunkSize, scratch);
	}
}
