...
public interface CompressionCodec {
	...
	/**
	 * Legacy form: assumes the entire array is the compressed payload.
	 */
	byte[] decompress(byte[] compressed, int originalLength) throws IOException;

	/**
	 * Decompresses a specific range of the compressed data buffer, so input
	 * buffers (e.g. reused network buffers) can be processed zero-copy.
	 */
