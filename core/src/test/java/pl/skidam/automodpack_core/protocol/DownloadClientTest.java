...
class DownloadClientTest {
    ...
    void rejectsCaIssuedCertificateForDnsFallback() throws Exception {
        ...
        X509Certificate leaf = issueCertificate(
                ...
                new X500Name("CN=AutoModpack CA-issued Certificate"),
                null,
                NetUtils.generateKeyPair(), issuerKeyPair,
    ...
    void acceptsExpiredSelfSignedCertificateForDnsFallback() throws Exception {
        ...
        X509Certificate certificate = issueCertificate(
                subject, subject, null, keyPair, keyPair,
    ...
    void authenticatesOriginWhileConnectingToDifferentRoute() throws Exception {
        ...
        X509Certificate certificate = issueCertificate(
                subject, subject, "origin.example", keyPair, keyPair,
                Instant.now().minusSeconds(60), Instant.now().plusSeconds(3600));

        withTlsServer(keyPair, certificate, port -> {
            var addresses = new Jsons.ModpackAddresses(
                    InetSocketAddress.createUnresolved("route.example", port),
                    InetSocketAddress.createUnresolved("origin.example", 25565),
                    false);
    ...
    void rejectsCertificateValidOnlyForAdvertisedRoute() throws Exception {
        ...
        X509Certificate certificate = issueCertificate(
                subject, subject, "route.example", keyPair, keyPair,
                Instant.now().minusSeconds(60), Instant.now().plusSeconds(3600));
        ...
        withTlsServer(keyPair, certificate, port -> {
            ...
            assertThrows(IOException.class,
                    () -> new PreValidationConnection(route, addresses, clientContext(certificate)));
        });
    }

    private static void withTlsServer(KeyPair keyPair, X509Certificate certificate, ThrowingPortConsumer test) throws Exception {
        SSLServerSocket server = (SSLServerSocket) serverContext(keyPair, certificate)
                .getServerSocketFactory().createServerSocket(0, 1, InetAddress.getLoopbackAddress());
