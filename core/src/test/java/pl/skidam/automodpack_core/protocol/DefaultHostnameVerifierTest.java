...
class DefaultHostnameVerifierTest {
	...
	static void setUp() throws Exception {
		...
		certificateWithSAN = createCertificate(keyPair, "CN=example.com", new String[]{"example.com", "www.example.com", "api.example.com"});

		// Create certificate with Common Name only
		certificateWithCNOnly = createCertificate(keyPair, "CN=example.org", null);

		// Create certificate with wildcard domain
		certificateWithWildcard = createCertificate(keyPair, "CN=*.example.net", new String[]{"*.example.net"});
	}
	...
	private static X509Certificate createCertificate(KeyPair keyPair, String subjectDN, String[] subjectAltNames) throws Exception {
		...
		X509v3CertificateBuilder certBuilder = new JcaX509v3CertificateBuilder(issuerName, serialNumber, notBefore, notAfter, subjectName, keyPair.getPublic());
