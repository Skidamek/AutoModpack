...
public class ServerLoginNetworking {
	...
	public interface LoginQueryResponseHandler {
		 ...
		 * Modification to the game should be {@linkplain net.minecraft.util.thread.BlockableEventLoop#submit(Runnable) scheduled} using the provided Minecraft
		 * client instance.
		 *
		 * <p>
		 ...
		 *            the server
		 * @param handler
		 *            the network handler that received this packet, representing the player/client who sent the response
		 * @param understood
		 *            whether the client understood the packet
		 * @param buf
		 *            the payload of the packet
		 * @param synchronizer
		 *            the synchronizer which may be used to delay log-in till a {@link Future} is completed.
		 */
		void receive(MinecraftServer server, ServerLoginPacketListenerImpl handler, boolean understood, FriendlyByteBuf buf, LoginSynchronizer synchronizer,
