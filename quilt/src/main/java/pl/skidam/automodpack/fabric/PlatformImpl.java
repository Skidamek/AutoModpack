        ...
        if (!Platform.isModLoaded("quilted_fabric_api") && !Platform.isModLoaded("fabric")) { // QFAPI
            ...
            if (Platform.getEnvironmentType().equals("SERVER")) {
                if (!AutoModpack.serverConfig.updateCheck) {
                    AutoModpack.LOGGER.warn("AutoModpack update check is disabled, you need to manually install quilted fabric api!");
                    return;
                }
            }
            ...
            try {
                ...
                String localChecksum = downloadInstance.download(qfapi.modrinthAPIdownloadUrl, new File(modsPath.toFile() + File.separator + qfapi.modrinthAPIfileName)); // Download it
