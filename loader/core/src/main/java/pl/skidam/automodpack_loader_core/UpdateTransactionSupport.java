...
public final class UpdateTransactionSupport {
	...
	private static void applyLauncherMetadata(UpdateTransaction transaction, Jsons.ModpackContentFields manifest) throws IOException {
		if (!transaction.restartReasons.contains(RestartReason.CHANGED_LOADER_VERSION)) return;
		if (Constants.clientConfig == null) {
			Constants.clientConfig = ConfigTools.read(storage().clientConfigFile(), Jsons.ClientConfigFieldsV3.class)
