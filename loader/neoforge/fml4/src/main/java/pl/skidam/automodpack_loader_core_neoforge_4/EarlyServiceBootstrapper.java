 ...
 * Everything generation-specific in the body (SecureJar, cpw.mods.cl) sits behind that guard.
 */
public class EarlyServiceBootstrapper implements GraphicsBootstrapper {
	...
	private boolean buildAndRegister(List<Path> jars, ModuleLayer serviceLayer) {
		try {
			...
			Constants.LOGGER.error("[AutoModpack] Could not build a service layer for early-service jar(s) {}", jars.stream().map(Path::getFileName).toList(),
					t);
			return false;
		}
	}
