...
def test_fake_new_repair_and_preservation_ui_states(make_ctx):
    ...
    assert any(button["text"] == "Keep changes in config/pack-shared-editable.txt" and not button.get("checked") for button in buttons)
    ...
    assert any(button["text"] == "Keep changes in config/pack-shared-editable.txt" and button.get("checked") for button in bridge.gui()["buttons"])
    bridge.click(95)
